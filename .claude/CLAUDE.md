...
app/monthly_audit.py    → 月度因子审计 (A1-A6 稳健性检验 + 稳定性监测层 + Part B 新因子发现, 输出 audit_report.md)
...
src/models/factor_optimizer.py → 三漏斗因子筛选 + Evidence-based 健康监测层 (Grade/Confidence/Action/VersionRecommendation)
...
## 数据流
AKShare(指数/融资/北向/沪深300/M2) + Sina(ETF实时) → resample(W-FRI) → FactorPool(14核心因子 + 3外部候选) → 三漏斗筛选(稀疏度→收益→CI) + Evidence-based 健康监测层(Grade/Confidence/Action) → Version Recommendation → MODEL_CONFIGS(V5.2: 4因子 L1=3.0/M1=2.5/S3=2.0/V1=2.0) → evaluate_signal() → Score + n_factors_tier Armed → signal_tier(hold/weak/standard/strong) → SQLite

factor_optimizer.py 架构: screen_factors() 三漏斗筛选 → factor_health_analysis() 收集多维Evidence(滚动窗口/触发CV/A1漂移/频率漂移/uplift衰减)综合输出 Grade+Confidence+Action → version_recommendation() 聚合因子健康给出版本决策(Keep Current/Recommend V5.3 Review)。HEALTH_ACTIONS 映射表定义 Grade→行动建议。

外部数据(north_w/hs300_w/m2_w)已接入数据管线，3个候选因子(S4_north_diverge/E1_market_bear/E2_m2_accel)在RULE_DEFS中有定义但未加入MODEL_CONFIGS——经五阶段筛选均未通过（与现有因子高度共触发或样本不足），代码骨架保留供后续审计重新评估。跨市场因子(XBI/IBB)和产业链因子(化工/食品/房地产)经测试均无独立预测力。

当前因子健康: L1=Stable(Medium), S3=Stable(High), M1=Regime-dependent(Medium), V1=Declining(High,衰减比0.13)。版本决策: Keep Current — 重点观察(需连续2次Declining才启动V5.3)。版本升级准入: 连续2次审计同一因子Declining或任一次Unstable。
